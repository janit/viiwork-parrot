...
func (t *Torrent) close(wg *sync.WaitGroup) {
	...
	// viiwork-parrot patch: drop this torrent's webseed requests from the
	// client-global bookkeeping synchronously, here, while cl.lock() is
	// still held and before this torrent is removed from t.cl.torrents
	// below. Previously each request only removed itself later, from its
	// own goroutine once it noticed its context was cancelled, racing the
	// client's immediately-rescheduled webseed-request-update timer (also
	// armed by this same close(), via the peer close below) with no
	// ordering guarantee against that timer's consistency check in
	// updateWebseedRequests. See VIIWORK-PARROT-PATCH.md.
	t.dropActiveWebseedRequestsLocked()
	t.iterPeers(func(p *Peer) {
		p.close()
	})
	if t.storage != nil {
...
// viiwork-parrot patch: see close()'s call site and VIIWORK-PARROT-PATCH.md. Must be
// called with cl.lock() held (true of every close() call site). Collects
// the keys first, rather than deleting from activeRequests while ranging
// over it directly, purely for readability; Go permits either.
func (t *Torrent) dropActiveWebseedRequestsLocked() {
	for _, ws := range t.webSeeds {
		reqs := make([]*webseedRequest, 0, len(ws.activeRequests))
		for wr := range ws.activeRequests {
			reqs = append(reqs, wr)
		}
		for _, wr := range reqs {
			ws.deleteActiveRequest(wr)
		}
	}
}
