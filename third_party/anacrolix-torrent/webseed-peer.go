...
// viiwork-parrot patch: made idempotent (was g.MustDelete, which panics on an
// absent key) because Torrent.close may already have synchronously removed
// this entry via dropActiveWebseedRequestsLocked before this request's own
// goroutine (sliceProcessor) gets here. See VIIWORK-PARROT-PATCH.md.
func (ws *webseedPeer) deleteActiveRequest(wr *webseedRequest) {
	if _, ok := ws.activeRequests[wr]; !ok {
		return
	}
	...
	delete(cl.activeWebseedRequests, ws.getRequestKey(wr))
	ws.peer.updateExpectingChunks()
}
